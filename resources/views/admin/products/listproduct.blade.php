    ...
    <div class="main-content">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select class="form-select" id="loc_category" name="category">
                                                    ...
                                                    @foreach ($category as $item)
                                                        <option
                                                            value="{{ route('admin.product.filter', ['id' => $item->id]) }}">
                                                            {{ $item->name }}</option>
                                                    @endforeach
                                                ...
                                                <tbody>
                                                    ...
                                                    @foreach ($products as $key => $value)
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                @if (isset($value->images[0]))
                                                                    <img style="width: 100px; height: 75px;"
                                                                        src="{{ asset($value->images[0]->image_path) }}"
                                                                        alt="">
                                                                @endif
                                                            ...
                                                            <td>
                                                                <div class="form-group">
                                                                    <select class="form-select category" name="category">
                                                                        @foreach ($category as $item)
                                                                            <option
                                                                                {{ $value->category_id == $item->id ? 'selected' : '' }}
                                                                                data-idProduct="{{ $value->id }}"
                                                                                value="{{ $item->id }}">
                                                                                {{ $item->name }}</option>
                                                                        @endforeach
                                                                    </select>
                                                            ...
                                                            <td>
                                                                <div class="form-group">
                                                                    <select class="form-select status trang-thai"
                                                                        id="status">
                                                                        <option
                                                                            {{ $value->status == 'published' ? 'selected' : '' }}
                                                                            data-idProduct="{{ $value->id }}"
                                                                            value="published">Được
                                                                            phát hành</option>
                                                                        <option
                                                                            {{ $value->status == 'inactive' ? 'selected' : '' }}
                                                                            data-idProduct="{{ $value->id }}"
                                                                            value="inactive">Không
                                                                            hoạt động</option>
                                                                        <option
                                                                            {{ $value->status == 'scheduled' ? 'selected' : '' }}
                                                                            data-idProduct="{{ $value->id }}"
                                                                            value="scheduled">Lên
                                                                            kế
                                                                            hoạch</option>
                                                                ...
                                                                @if ($role['user']['role_id'] === 1)
                                                                    <td>
                                                                        <div class="toggle-container">
                                                                            <input type="checkbox"
                                                                                id="toggle{{ $value->id }}"
                                                                                {{ $value->approve_product == 1 ? 'checked' : '' }}
                                                                                value="{{ $value->id }}"
                                                                                class="toggle-checkbox-approve">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                ...
                // change duyệt sản phấm
                const checkboxesApprove = document.querySelectorAll('.toggle-checkbox-approve');
                checkboxesApprove.forEach((checkbox, index) => {
                    checkbox.addEventListener('change', function(event) {
                        const value = event.target.value;
                        const isChecked = this.checked ? 1 : 0;
                        // Gọi API với fetch
                        ...
                        formData.append('_token', '{{ csrf_token() }}');
                        $.ajax({
                            url: '{{ route('admin.product.approve') }}',
                            type: 'POST',
                            data: formData,
                            contentType: false,
                            processData: false,
        ...
        <style scoped>
            ...
            .text-product {
                ...
                display: -webkit-box;
                /* Sử dụng box để giới hạn số dòng */
                -webkit-line-clamp: 2;
                /* Hiển thị tối đa 2 dòng */
                -webkit-box-orient: vertical;
                /* Định hướng dọc cho box */
                line-height: 1.2;
